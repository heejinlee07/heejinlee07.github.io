...
여기서 `나중`은 `지금`의 직후가 아니며, `지금` 끝낼 수 없는 작업은 비동기적으로 처리되기 때문에 프로그램을 중단하지 않는다. AJAX를 예로 들자면 이 함수는 비동기적으로 어떠한 task를 `지금` 요청하고 `나중`에 결과를 받고, `지금`부터 `나중`까지 기다리는 최적의 방법은 `콜백 함수`를 이용하는 것이다.
