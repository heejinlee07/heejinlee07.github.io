...
- concat은 원본 배열을 직접 변경하지 않고, `반환값`을 반드시 변수에 할당해야한다. 반면에 push, unshift는 원본 배열을 직접 변경하며, `원본 배열`을 반드시 변수에 할당해야 한다.
...
- 두 번째9 인수로 검색을 시작할 인덱스를 전달한다. 생략하면 기본값 0이 설정된다. 음수를 전달하면 length와 음수 인덱스를 합산하여 검색 시작 인덱스를 설정한다.
